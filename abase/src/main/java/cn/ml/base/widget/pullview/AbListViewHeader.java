 ...
 */
package cn.ml.base.widget.pullview;
...
public class AbListViewHeader extends LinearLayout {
	...
	private void initView(Context context) {
		...
		MLViewUtil.setPadding(headerView, 0, 10, 0, 10);
		
		//显示箭头与进度
		FrameLayout headImage =  new FrameLayout(context);
		arrowImageView = new ImageView(context);
		//从包里获取的箭头图片
		arrowImage =BitmapFactory.decodeResource(getResources(), R.drawable.pullview_arrow);
		...
		layoutParamsWW.width = MLViewUtil.scale(mContext, 30);
		layoutParamsWW.height = MLViewUtil.scale(mContext, 30);
		
		//箭头
		LayoutParams layoutParamsHI = new LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);
		layoutParamsHI.gravity = Gravity.CENTER;
		layoutParamsHI.width = MLViewUtil.scale(mContext, 45);
		layoutParamsHI.height = MLViewUtil.scale(mContext, 45);
		
		headImage.addView(arrowImageView,layoutParamsHI);
...
//		tipsTextview.setTextColor(Color.rgb(107, 107, 107));
		tipsTextview.setTextColor(Color.rgb(28, 28, 28));
		headerTimeView.setTextColor(Color.rgb(107, 107, 107));
		MLViewUtil.setTextSize(tipsTextview,26);
		MLViewUtil.setTextSize(headerTimeView,22);
		...
		layoutParamsWW3.rightMargin = MLViewUtil.scale(mContext, 15);
		...
		switch(state){
			case STATE_NORMAL:
				...
				if(lastRefreshTime==null){
					lastRefreshTime = MLDateUtil.getCurrentDate(MLDateUtil.dateFormatHMS);
