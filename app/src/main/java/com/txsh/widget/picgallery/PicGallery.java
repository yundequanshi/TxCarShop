...
public class PicGallery extends Gallery {
    ...
    public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX,
        ...
        if (view instanceof MyImageView) {
            ...
            float min_distance = BaseApplication.screenWidth / 4f;
            Logger.d(DEBUG, "xdistance=" + xdistance + ",min_distance="
                    + min_distance);
            ...
            if ((int) width <= BaseApplication.screenWidth
                    && (int) height <= BaseApplication.screenHeight)// 如果图片当前大小<屏幕大小，直接处理滑屏事件
            {
                ...
                if (distanceX > 0)// 向左滑动
                {
                    if (r.left > 0 || right < BaseApplication.screenWidth) {// 判断当前ImageView是否显示完全
                        super.onScroll(e1, e2, distanceX, distanceY);
                    } else {
                ...
                {
                    if (r.right < BaseApplication.screenWidth || left > 0) {
                        super.onScroll(e1, e2, distanceX, distanceY);
                    } else {
                        imageView.postTranslate(-distanceX, -distanceY);
                    }
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_UP:
                ...
                if (view instanceof MyImageView) {
                    ...
                    // + height + ",screenWidth="
                    // + BaseApplication.screenWidth + ",screenHeight="
                    // + BaseApplication.screenHeight);
                    if ((int) width <= BaseApplication.screenWidth
                            && (int) height <= BaseApplication.screenHeight)// 如果图片当前大小<屏幕大小，判断边界
                    {
                        break;
                    }
                    float v[] = new float[9];
                    ...
                    if (top < 0 && bottom < BaseApplication.screenHeight) {
//					imageView.postTranslateDur(-top, 200f);
                        imageView.postTranslateDur(BaseApplication.screenHeight
                                - bottom, 200f);
                    }
                    if (top > 0 && bottom > BaseApplication.screenHeight) {
//					imageView.postTranslateDur(BaseApplication.screenHeight
//							- bottom, 200f);
                    ...
                    if (left < 0 && right < BaseApplication.screenWidth) {
//					imageView.postTranslateXDur(-left, 200f);
                        imageView.postTranslateXDur(BaseApplication.screenWidth
                                - right, 200f);
                    }
                    if (left > 0 && right > BaseApplication.screenWidth) {
//					imageView.postTranslateXDur(BaseApplication.screenWidth
