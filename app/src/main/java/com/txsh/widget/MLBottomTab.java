...
public class MLBottomTab extends LinearLayout {
    ...
    private AbUnSlideViewPager _viewPager;
     ...
     */
    public void addItemViews(List<String> tabTexts, List<Drawable> drawables, AbUnSlideViewPager viewpager) {
    ...
    private void addTab(String text, int i, Drawable drawable) {
        ...
        if (i == 0) {
            //初始化时，默认选中第一个
            tabView.setData(i, text, Color.parseColor("#0c71d7"), tabItemDrawableList.get(1));
        } else {
            tabView.setData(i, text, Color.parseColor("#3c3c3c"), drawable);
        }
    ...
    private OnClickListener mTabClickListener = new OnClickListener() {
        ...
        private void setCurrentItem(int index) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (isSelected) {
                    //选中
                    tabView.setData(Color.parseColor("#0c71d7"), tabItemDrawableList.get(index * 2 + 1));
                } else {
                    tabView.setData(Color.parseColor("#3c3c3c"), tabItemDrawableList.get(i * 2));
                }
            }
            //选择viewPager 的当前页
