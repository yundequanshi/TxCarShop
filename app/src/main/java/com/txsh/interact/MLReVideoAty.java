...
public class MLReVideoAty extends BaseAct implements
...
//		}
    ...
    private void handleSurfaceChanged() {
        ...
        // 获取摄像头的所有支持的分辨率
        /*List<Size> resolutionList = EasyUtils.getResolutionList(mCamera);
        if (resolutionList != null && resolutionList.size() > 0) {
			Collections.sort(resolutionList, new EasyUtils.ResolutionComparator());
    ...
    public void onClick(View view) {
        if (view.getId() == R.id.switch_btn) {
            switchCamera();
        } else if (view.getId() == R.id.recorder_start) {
            if (!startRecording())
                return;
            ...
            chronometer.start();
        } else if (view.getId() == R.id.recorder_stop) {
...
//		}
    ...
    private boolean initRecorder() {
        ...
        // 设置视频文件输出的路径
        localPath = MLFolderUtils.getDiskFile(this,"video") + "/"
