 ...
 */
public class TXShopOrderDshAdapter extends MLAdapterBase<TXShopOrderRes.TXShopOrderData> {



    public TXShopOrderDshAdapter(Context context, int viewXml) {
    ...
    protected void setItemData(View view, final TXShopOrderRes.TXShopOrderData data, final int position) {
        ...
        mBtnAffirm.setText("确认收货");
        mBtnLogistics.setText("物流信息");
        mBtnAffirm.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {

                TXEventModel model = new TXEventModel(MLConstants.EVENT_PARAM_ORDER_AFFIRM, data);
                EventBus.getDefault().post(model);
            }
        });
        mBtnLogistics.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                TXEventModel model = new TXEventModel(MLConstants.EVENT_PARAM_ORDER_LOGISTICAL, data);
                EventBus.getDefault().post(model);
            }
        });


        if(MLStrUtil.compare(data.orderFormState, "待发货")){
