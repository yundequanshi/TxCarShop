 ...
 * Description:
 */
public class ZMCrashHandler implements Thread.UncaughtExceptionHandler {

    private static final String TAG = ZMCrashHandler.class.getSimpleName();

    //系统默认的UncaughtException处理类
    private Thread.UncaughtExceptionHandler mDefaultHandler;
    //CrashHandler实例
    private static ZMCrashHandler INSTANCE = new ZMCrashHandler();
    //程序的Context对象
     ...
     */
    private ZMCrashHandler() {
    }

    /**
     * 获取CrashHandler实例 ,单例模式
     */
    public static ZMCrashHandler getInstance() {
        return INSTANCE;
    }
...
//    	CrashReport.postCatchedException(ex);
    ...
    private void sendError(){
    	String url = APIConstants.API_DEFAULT_HOST+"/exception/save";
    	RequestParams params = new RequestParams();
    	//params.addHeader("name", "value");
    	params.addQueryStringParameter("msg", errorMsg);

   // 	params.addBodyParameter("msg", errorMsg);
...
/*        new Thread() {
    ...
    private File getCrashLogFolder() {
        if (_crashLogDirFile == null) {
            ...
            _crashLogDirFile = new File(SDCardRoot, BaseApplication.getInstance().getString(R.string.app_name)+"/日志/");
