...
public class EaseChatFragment extends EaseBaseFragment implements EMMessageListener {
    ...
    protected HxUser mDataUser;
    protected GroupData mDataGroup;
    ...
    static final int ITEM_TRANSFER = 4;
    //  位置 R.string.attach_location     R.drawable.ease_chat_location_selector
    protected int[] itemStrings = {R.string.attach_take_pic, R.string.attach_picture, R.string.attach_location};
    protected int[] itemdrawables = {R.drawable.ease_chat_takepic_selector,
            R.drawable.ease_chat_image_selector, R.drawable.ease_chat_location_selector};
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        if (chatType == EaseConstant.CHATTYPE_GROUP) {
            ...
            if (toChatUsername == null) {
                //群列表跳转
                mDataGroup = (GroupData) fragmentArgs.getSerializable(EaseConstant.EXTRA_GROUP);
                toChatUsername = mDataGroup.emgId;
            }
        } else {
            //单聊
            // 会话人或群组id
            mDataUser = (HxUser) fragmentArgs.getSerializable(Contants.EXTRA_USER);
            if (mDataUser == null) {
                //数据库查询 陌生人信息
                mDataUser = MLDBUtils.getFirst(Selector.from(HxUser.class).where("emId", "=", toChatUsername));
                if (mDataUser != null) {
                    toChatUsername = mDataUser.emId;
                }
            } else {
                toChatUsername = mDataUser.emId;
            }
    ...
    protected void setUpView() {
        if (chatType == EaseConstant.CHATTYPE_SINGLE) { // 单聊
            ...
            if (mDataUser != null) {
                //把用户更新到数据库
                HxUser user = MLDBUtils.getFirst(Selector.from(HxUser.class).where("emId", "=", mDataUser.emId));
                MLDBUtils.saveOrUpdate(mDataUser);
                titleBar.setTitle(mDataUser.name);
          /*  if(EaseUserUtils.getUserInfo(toChatUsername) != null){
            ...
            if (chatType == EaseConstant.CHATTYPE_GROUP) {
                ...
                if (mDataGroup == null) {
                    ...
                    titleBar.setTitle(mDataGroup.name);
                }
                // 监听当前会话的群聊解散被T事件
        ...
        messageList.setItemClickListener(new EaseChatMessageList.MessageListItemClickListener() {
            ...
            public void onUserAvatarClick(String username) {
                if (chatFragmentListener != null) {
                    chatFragmentListener.onAvatarClick(username,toChatUsername);
                }
            }
    ...
    public interface EaseChatFragmentListener {
         ...
         * @param username
         */
        void onAvatarClick(String username, String toUserName);
